:: ===========================================================================
:: Nom       : ExportDrivers.bat
:: Projet    : WinMigrate
:: Author    : PS81FRT
:: GitHub    : https://github.com/ps81frt/winmigrate
:: Version   : 1.0.0
:: Licence   : MIT
:: Synopsis  : Export all third-party drivers to a destination folder using DISM
...
set "LOG_FILE=%TEMP%\WinMigrate_ExportDrivers_%DATE:~10,4%%DATE:~4,2%%DATE:~7,2%.log"
set /p "CONFIRM=Confirm driver export to %DEST%? (!_YESNO!) : "
if /i not "%CONFIRM%"=="%_YES%" exit /b 1
dism /Online /Export-Driver /Destination:"%DEST%" >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
    echo [ERROR] Driver export failed. See %LOG_FILE%
    exit /b 1
)
echo [OK] Driver export complete. See %LOG_FILE%
exit /b 0