...
:: Version   : 1.0.0
:: Licence   : MIT
:: Synopsis  : Export system and user PATH environment variables as .reg files
...
set "LOG_FILE=%TEMP%\WinMigrate_ExportPath_%DATE:~10,4%%DATE:~4,2%%DATE:~7,2%.log"
set /p "CONFIRM=Confirm PATH export to %DEST%? (!_YESNO!) : "
if /i not "%CONFIRM%"=="%_YES%" exit /b 1
reg export "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Environment" "%DEST%\PATH_System.reg" /y >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
    echo [ERROR] PATH export failed. See %LOG_FILE%
    exit /b 1
)
reg export "HKCU\Environment" "%DEST%\PATH_User.reg" /y >> "%LOG_FILE%" 2>&1
echo [OK] PATH export complete. See %LOG_FILE%
exit /b 0