...
:: Version   : 1.0.0
:: Licence   : MIT
:: Synopsis  : Verify BCD store entries using bcdedit
...
:ADMIN_OK
set /p "CONFIRM=Confirm BCD store verification? (!_YESNO!) : "
if /i not "%CONFIRM%"=="%_YES%" exit /b 1
bcdedit > "%TEMP%\WinMigrate_VerifyBcd.txt" 2>&1
if errorlevel 1 (
    echo [ERROR] bcdedit failed.
    exit /b 1
)
type "%TEMP%\WinMigrate_VerifyBcd.txt"
echo [OK] BCD verification complete. See %TEMP%\WinMigrate_VerifyBcd.txt
exit /b 0