:: ===========================================================================
:: Nom       : ImportFirewall.bat
:: Projet    : WinMigrate
:: Author    : PS81FRT
:: GitHub    : https://github.com/ps81frt/winmigrate
:: Version   : 1.0.0
:: Licence   : MIT
:: Synopsis  : Import Windows Firewall rules from a .wfw backup file
:: Usage     : Run as Administrator - arg: [SourceFile.wfw]
:: ===========================================================================
@echo off
chcp 437 > nul
...
set "SOURCE=%~1"
if "%SOURCE%"=="" (
    echo Usage: %~nx0 [SourceFile.wfw]
    exit /b 1
)
set "LOG_FILE=%TEMP%\WinMigrate_ImportFirewall_%DATE:~10,4%%DATE:~4,2%%DATE:~7,2%.log"
set /p "CONFIRM=Confirm firewall rules import from %SOURCE%? (!_YESNO!) : "
if /i not "%CONFIRM%"=="%_YES%" exit /b 1
netsh advfirewall import "%SOURCE%" > "%LOG_FILE%" 2>&1
if errorlevel 1 (
    echo [ERROR] Firewall import failed. See %LOG_FILE%
    exit /b 1
)
echo [OK] Firewall rules imported successfully. See %LOG_FILE%
exit /b 0