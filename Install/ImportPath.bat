...
:: Version   : 1.0.0
:: Licence   : MIT
:: Synopsis  : Import PATH environment variables from a .reg file
:: Usage     : Run as Administrator - arg: [SourceFile.reg]
:: ===========================================================================
@echo off
chcp 437 > nul
...
set "SOURCE=%~1"
if "%SOURCE%"=="" (
    echo Usage: %~nx0 [SourceFile.reg]
    exit /b 1
)
set "LOG_FILE=%TEMP%\WinMigrate_ImportPath_%DATE:~10,4%%DATE:~4,2%%DATE:~7,2%.log"
set /p "CONFIRM=Confirm PATH import from %SOURCE%? (!_YESNO!) : "
if /i not "%CONFIRM%"=="%_YES%" exit /b 1
reg import "%SOURCE%" >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
    echo [ERROR] PATH import failed. See %LOG_FILE%
    exit /b 1
)
echo [OK] PATH import complete. See %LOG_FILE%
exit /b 0