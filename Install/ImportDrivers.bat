:: ===========================================================================
:: Nom       : ImportDrivers.bat
:: Projet    : WinMigrate
:: Author    : PS81FRT
:: GitHub    : https://github.com/ps81frt/winmigrate
:: Version   : 1.0.0
:: Licence   : MIT
:: Synopsis  : Import (add) drivers online from a source folder using DISM
...
set "LOG_FILE=%TEMP%\WinMigrate_ImportDrivers_%DATE:~10,4%%DATE:~4,2%%DATE:~7,2%.log"
set /p "CONFIRM=Confirm driver import from %SOURCE%? (!_YESNO!) : "
if /i not "%CONFIRM%"=="%_YES%" exit /b 1
dism /Online /Add-Driver /Driver:"%SOURCE%" /Recurse >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
    echo [ERROR] Driver import failed. See %LOG_FILE%
    exit /b 1
)
echo [OK] Driver import complete. See %LOG_FILE%
exit /b 0