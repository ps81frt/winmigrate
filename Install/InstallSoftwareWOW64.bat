:: ===========================================================================
:: Nom       : InstallSoftwareWOW64.bat
:: Projet    : WinMigrate
:: Author    : PS81FRT
:: GitHub    : https://github.com/ps81frt/winmigrate
:: Version   : 1.0.0
:: Licence   : MIT
:: Synopsis  : Run all 32-bit (WOW64) software installers found in a source folder
...
set "LOG_FILE=%TEMP%\WinMigrate_InstallSoftwareWOW64_%DATE:~10,4%%DATE:~4,2%%DATE:~7,2%.log"
set /p "CONFIRM=Confirm running all 32-bit installers from %SOURCE%? (!_YESNO!) : "
if /i not "%CONFIRM%"=="%_YES%" exit /b 1
echo Starting 32-bit software installs from %SOURCE% > "%LOG_FILE%"
for %%F in ("%SOURCE%\*.exe") do (
    echo Installing: %%~nxF >> "%LOG_FILE%"
    "%WINDIR%\SysWOW64\cmd.exe" /c start /wait "" "%%F" /S /quiet /norestart >> "%LOG_FILE%" 2>&1
)
for %%F in ("%SOURCE%\*.msi") do (
    echo Installing: %%~nxF >> "%LOG_FILE%"
    msiexec /i "%%F" /quiet /norestart >> "%LOG_FILE%" 2>&1
)
echo [OK] 32-bit software installation complete. See %LOG_FILE%
exit /b 0