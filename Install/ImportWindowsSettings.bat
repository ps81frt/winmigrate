:: ===========================================================================
:: Nom       : ImportWindowsSettings.bat
:: Projet    : WinMigrate
:: Author    : PS81FRT
:: GitHub    : https://github.com/ps81frt/winmigrate
:: Version   : 1.0.0
:: Licence   : MIT
:: Synopsis  : Import Windows settings by loading all .reg files from a folder
...
set "LOG_FILE=%TEMP%\WinMigrate_ImportSettings_%DATE:~10,4%%DATE:~4,2%%DATE:~7,2%.log"
set /p "CONFIRM=Confirm Windows settings import from %SOURCE%? (!_YESNO!) : "
if /i not "%CONFIRM%"=="%_YES%" exit /b 1
set "ERR=0"
for %%F in ("%SOURCE%\*.reg") do (
    echo Importing %%F >> "%LOG_FILE%"
    reg import "%%F" >> "%LOG_FILE%" 2>&1
    if errorlevel 1 set "ERR=1"
)
if "%ERR%"=="1" (
    echo [WARN] Some registry import operations failed. See %LOG_FILE%
)
echo [OK] Windows settings import complete. See %LOG_FILE%
exit /b 0